...
// .wrangler/tmp/bundle-nzplNT/checked-fetch.js
...
// .wrangler/tmp/bundle-nzplNT/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();
...
// .wrangler/tmp/bundle-nzplNT/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var HKMetaDataSchema = object({
  HKAverageMETs: string2(),
  HKElevationAscended: string2(),
  HKIndoorWorkout: number(),
  HKTimeZone: string2(),
  HKWeatherHumidity: string2(),
  HKWeatherTemperature: string2()
});
...
var ActivityCycling = class extends K {
  async handle(request, env, context, data) {
    ...
    while (!processedCycling) {
      ...
      const responseData = await response.json();
      const validData = parse2(array(ActivitySchema), responseData);
      if (validData.at(0).name.startsWith("Cycling")) {
...
var ActivityRunning = class extends K {
  async handle(request, env, context, data) {
    ...
    while (!processedRunning) {
      ...
      const responseData = await response.json();
      const validData = parse2(array(ActivitySchema), responseData);
      if (validData.at(0).name.startsWith("Running")) {
...
var CarbohydratesToday = class extends K {
  async handle(request, env, context, data) {
    ...
    const responseData = await response.json();
    const validData = parse2(array(CarbohydrateSchema), responseData);
    return validData;
  }
};
